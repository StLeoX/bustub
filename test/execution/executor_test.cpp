...
TEST_F(ExecutorTest, /*DISABLED_*/ SimpleSeqScanTest) {
  ...
  auto col_a_idx = out_schema->GetColIdx("colA");
  LOG_DEBUG("col_a_idx is %d", col_a_idx);
  for (const auto &tuple : result_set) {
    ASSERT_TRUE(tuple.GetValue(out_schema, col_a_idx).GetAs<int32_t>() < 500);
  }

  auto col_b_idx = out_schema->GetColIdx("colB");
  LOG_DEBUG("col_b_idx is %d", col_b_idx);
  for (const auto &tuple : result_set) {
    ASSERT_TRUE(tuple.GetValue(out_schema, col_b_idx).GetAs<int32_t>() < 10);
  }
}
...
TEST_F(ExecutorTest, /*DISABLED_*/ SimpleUpdateTest) {
...
// deb(assert) values.size() != schema->GetColumnCount()
...
// deb()
TEST_F(ExecutorTest, /*DISABLED_*/ SimpleLimitTest) {
  auto *table_info = GetExecutorContext()->GetCatalog()->GetTable("test_3");
  auto &schema = table_info->schema_;
...
TEST_F(ExecutorTest, /*DISABLED_*/ SimpleDistinctTest) {
