...
class Trie {
 ...
 public:
   ...
   * character.
   */
  Trie() : root_(std::make_unique<TrieNode>('\0')), latch_(){};
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (size_t i = 0; i < n - 1; ++i) {
      if (cur->HasChild(key[i])) {
        ...
        cur = cur->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]))->get();
      }
    }
    // terminal node
    if (!cur->HasChild(key[n - 1])) {
      cur->InsertChildNode(key[n - 1], std::make_unique<TrieNodeWithValue<T>>(key[n - 1], value));
      latch_.WUnlock();
      return true;
    } else {
  ...
  bool Remove(const std::string &key) {
    ...
    if (!cur->HasChild(key[n - 1])) {
      ...
      auto term = cur->GetChildNode(key[n - 1])->get();
      if (!term->HasChildren()) cur->RemoveChildNode(key[n - 1]);
      latch_.WUnlock();
      return true;
    }
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto term = dynamic_cast<TrieNodeWithValue<T> *>(cur);
    if (term == nullptr) {
      latch_.RUnlock();
      return {};  // *success == false;
    } else {
